...
class Tetris:
    ...
    def get_next_states(self):
        states = {}
        piece_id = self.ind
        print("block_id_DQN" + str(piece_id))
        # 次のピースを受け取る
        ...
        for i in range(num_rotations):
            ...
            for x in range(valid_xs + 1):
                ...
                if states[(x,i)][0] >= 3:
                    high_score_move = x
                    high_score_rotation = i
                    select_flag = True
                # print(self.get_state_properties(board))
            curr_piece = self.rotate(curr_piece)
        # pprint.pprint(states)
        # sys.exit()
        return states,high_score_move,high_score_rotation,select_flag
    ...
    # add module
    def setNextPiece(self,block_id):
        self.ind = block_id
        # mod
        # self.ind = 1
        self.piece = [row[:] for row in self.pieces[self.ind]]
